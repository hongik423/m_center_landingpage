...
const LEE_HUKYUNG_RESPONSES = {
  // 인사말 및 일반 응답
  greetings: [
    `안녕하세요! 기업의별 M-CENTER의 M센터장입니다.

    28년간 500개 이상 기업의 성장을 함께해온 이후경 경영지도사를 대표하여 상담해드리겠습니다.
    
    저희 M-CENTER는 단순한 컨설팅이 아닌, 실제 검증된 성과로 기업의 성장을 책임집니다.
    ...
    어떤 부분이 궁금하신지 편하게 말씀해주세요. 28년 경험을 바탕으로 정확하고 실용적인 답변을 드리겠습니다.
...
function identifyQuestionType(message: string): string {
  ...
  if (/ai|인공지능|생산성|효율성|자동화|일터혁신|상생컨설팅|고용노동부/i.test(lowerMessage)) {
...
function generateDirectResponse(message: string): string {
  ...
  switch (questionType) {
    ...
    default:
      return `안녕하세요! 기업의별 M-CENTER의 M센터장입니다.
      ...
      28년 경험을 바탕으로 실용적인 솔루션을 제시해드리겠습니다.
