/**
 * AI-CAMP 맞춤진단 및 상담신청 데이터 수집 웹앱 (완전판)
 * 
 * 주요 기능:
 * - CORS 문제 해결 및 에러 처리 개선
 * - 개인정보동의 체크박스 반영 (강화된 검증)
 * - AI 무료진단기 지원 - 새로운 필드 구조 반영
 * - "설립난도" → "사업성장단계" 변경 반영
 * - 4단계 투자 단계 지원 (Pre-Seed ~ Series B+)
 * - 기존 데이터 호환성 유지
 * 
 * 최종 업데이트: 2025.06.17
 */
...
function determineFormType(data) {
  ...
  if (data.formType === 'AI_무료진단' || 
      ...
      data.사업성장단계 || data.설립난도 || data.establishmentDifficulty ||
...
/**
 * AI 무료진단 폼 데이터 처리 - 사업성장단계 반영 (완전판)
 */
function processAIDiagnosisForm(sheet, data, output) {
  try {
    // 헤더가 없으면 헤더 추가 (사업성장단계로 업데이트)
    ...
    Logger.log('AI 무료진단 데이터 저장 성공: ' + JSON.stringify({
      회사명: data.회사명 || data.companyName,
      업종: data.업종 || data.industry,
      사업성장단계: data.사업성장단계 || data.설립난도 || data.establishmentDifficulty,
    ...
    return output.setContent(JSON.stringify({
      ...
      processedData: {
        businessStage: businessStageMap[data.사업성장단계 || data.설립난도 || data.establishmentDifficulty],
        industry: industryMap[data.업종 || data.industry] || data.업종 || data.industry
      }
    }));
...
/**
 * 테스트 함수 (스크립트 편집기에서 직접 실행 가능) - 사업성장단계 반영
 */
function testAIDiagnosisFunction() {
  const testData = {
    ...
    직원수: '6-10명',
    employeeCount: '6-10명',
    사업성장단계: '2단계', // 새로운 사업성장단계
    설립난도: '보통', // 기존 호환성
    establishmentDifficulty: '2단계',
...
function testFunction() {
  const testData = {
    ...
    mainConcerns: '디지털 전환과 비용 절감',
    expectedBudget: '1000-5000만원',
    urgency: '보통',
    contactName: '홍길동',
    contactPhone: '010-1234-5678',
  ...
  try {
    const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = getOrCreateSheet(spreadsheet, '기업진단신청');
    const output = ContentService.createTextOutput();
    output.setMimeType(ContentService.MimeType.JSON);
    
    const result = processDiagnosisForm(sheet, testData, output);
    console.log('일반 진단 테스트 결과:', result);
...
function addToSpreadsheet(data) {
  try {
    ...
    // 헤더가 없으면 추가
    if (sheet.getLastRow() === 0) {
      sheet.appendRow(['제출일시', '회사명', '업종', '담당자', '연락처', '이메일', '내용']);
    }
    ...
    Logger.log('레거시 데이터 저장 완료');
    return { success: true };
    
  } catch (error) {
    Logger.log('레거시 데이터 저장 오류: ' + error.toString());
 ...
 * 데이터 검증 유틸리티 함수
 */
function validateBusinessStage(stage) {
  const validStages = [
    '1단계', '2단계', '3단계', '4단계',
...
function validateIndustry(industry) {
  const validIndustries = [
    'manufacturing', 'it', 'service', 'retail', 'construction',
    'food', 'healthcare', 'education', 'finance', 'logistics',
    'consulting', 'other'
  ];
 ...
 * 로깅 유틸리티
 */
function logFormSubmission(formType, success, data) {
  const logEntry = {
    timestamp: new Date().toISOString(),
    formType: formType,
    success: success,
    companyName: data.회사명 || data.companyName || 'N/A',
    industry: data.업종 || data.industry || 'N/A',
    businessStage: data.사업성장단계 || data.설립난도 || data.establishmentDifficulty || 'N/A'
  };
  
  Logger.log('폼 제출 로그: ' + JSON.stringify(logEntry));
  return logEntry;
} 