...
const COMPANY_ADDRESS = '서울시 중구 명동';
...
// 📧 신청자 자동 회신 이메일 발송 (상담신청용)
function sendConsultationAutoReply(email, name, company, consultationType, uniqueId) {
  try {
    console.log('📧 상담신청 자동 회신 이메일 발송 시작:', { email, name, company, consultationType });
    
    const subject = `[M-CENTER] ${consultationType} 상담신청이 접수되었습니다 - ${company}`;
    
    const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
      <div style="text-align: center; margin-bottom: 30px;">
        <h2 style="color: #2563eb; margin: 0;">M-CENTER 경영지도센터</h2>
        <p style="color: #666; margin: 10px 0 0 0;">${consultationType} 상담신청 접수 완료</p>
      </div>
      
      <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h3 style="color: #1f2937; margin-top: 0;">안녕하세요 ${name}님,</h3>
        <p style="color: #374151; line-height: 1.6;">
          <strong>${company}</strong>의 <strong>${consultationType}</strong> 상담신청이 정상적으로 접수되었습니다.
        </p>
        
        <div style="background-color: white; padding: 15px; border-radius: 6px; margin: 15px 0;">
          <p style="margin: 0; color: #6b7280;"><strong>접수번호:</strong> ${uniqueId || 'CS' + Date.now().toString().slice(-6)}</p>
          <p style="margin: 5px 0 0 0; color: #6b7280;"><strong>접수일시:</strong> ${getCurrentKoreanTime()}</p>
        </div>
      </div>
      
      <div style="background-color: #ecfdf5; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h4 style="color: #065f46; margin-top: 0;">📞 상담 진행 안내</h4>
        <ul style="color: #374151; line-height: 1.6; padding-left: 20px;">
          <li><strong>1단계:</strong> 담당 전문가가 상담 내용을 검토합니다</li>
          <li><strong>2단계:</strong> 24시간 내에 전화 또는 이메일로 연락드립니다</li>
          <li><strong>3단계:</strong> 상담 일정을 조율하여 진행합니다</li>
        </ul>
      </div>
      
      <div style="background-color: #fef3c7; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h4 style="color: #92400e; margin-top: 0;">💼 상담 서비스</h4>
        <ul style="color: #374151; line-height: 1.6; padding-left: 20px;">
          <li>✅ 전문가 1:1 맞춤 상담</li>
          <li>✅ 경영 현황 진단 및 분석</li>
          <li>✅ 실무 솔루션 제안</li>
...
${company}의 ${consultationType} 상담신청이 정상적으로 접수되었습니다.

▶ 접수번호: ${uniqueId || 'CS' + Date.now().toString().slice(-6)}
▶ 접수일시: ${getCurrentKoreanTime()}

📞 상담 진행 안내:
1. 담당 전문가가 상담 내용을 검토합니다
2. 24시간 내에 전화 또는 이메일로 연락드립니다
3. 상담 일정을 조율하여 진행합니다
...
${COMPANY_NAME}
    ...
    console.log('✅ 상담신청 자동 회신 이메일 발송 완료:', email);
    return { success: true, recipient: email };
    
  } catch (error) {
    console.error('❌ 상담신청 자동 회신 이메일 발송 실패:', error);
    return { success: false, error: error.toString() };
  }
}
...
function processConsultationForm(data) {
  try {
    ...
    if (isDiagnosisLinked && (data.이메일 || data.email)) {
      try {
        updateDiagnosisConsultationStatus(data.이메일 || data.email);
      } catch (updateError) {
    ...
    if (userEmail && userName && companyName) {
      const autoReplyResult = sendConsultationAutoReply(userEmail, userName, companyName, consultationType, uniqueId);
      emailResults.autoReply = autoReplyResult;
    }
